...
# !!!!!!!!!!!!!!!!!!!Attention!!!!!!!!!!!!!!!!!!!   loading may take some time (w/ my computer: ~10min)
# loading Data
...
#rm(c_stopwords, graph, graph_log, sum_inc, sum_tweets, tweetwords, sentis, sum_t1, control, wave_names)
```
...
inc_fra <- all %>%
  ...
  mutate(after = if_else(date >= "2015-01-01" & date < "2015-01-14", tweets*1, 0)) %>%
...
inc_dnk <- all %>%
  filter(date >= "2015-02-07" & date <= "2015-02-21") %>%
  mutate(before = if_else(date >= "2015-02-07" & date < "2015-02-14", tweets*1, 0)) %>%
  mutate(after = if_else(date >= "2015-02-14" & date < "2015-02-21", tweets*1, 0)) %>%
