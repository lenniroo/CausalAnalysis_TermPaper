...
# !!!!!!!!!!!!!!!!!!!WARNING!!!!!!!!!!!!!!!!!!!   loading may take some time (w/ my computer: ~10min)
# loading Data
...
w1 <- getURL("https://raw.githubusercontent.com/lenniroo/CausalAnalysis_TermPaper/master/Scrape_hs1_jan_n.csv")
w1_tweets <- read.csv(text = w1)

# loading scraped tweets, wave 2 = attack on copenhagen, denmark in february (14th)
w2 <- getURL("https://raw.githubusercontent.com/lenniroo/CausalAnalysis_TermPaper/master/Scrape_hs2_feb_n.csv")
w2_tweets <- read.csv(text = w2)

#loading scraped tweets, wave 3 = attack on charleston, USA in june (17th)
w3 <- getURL("https://raw.githubusercontent.com/lenniroo/CausalAnalysis_TermPaper/master/Scrape_hs3_jun_n.csv")
w3_tweets <- read.csv(text = w3)

# loading scraped tweets, wave 4 = attack on Saint Quentin-Fallavier, France in June (26th)
w4 <- getURL("https://raw.githubusercontent.com/lenniroo/CausalAnalysis_TermPaper/master/Scrape_hs4_jun_n.csv")
w4_tweets <- read.csv(text = w4)

# loading scraped tweets, wave 5 = attack on Bielefeld, Germany in september (9th)
w5 <- getURL("https://raw.githubusercontent.com/lenniroo/CausalAnalysis_TermPaper/master/Scrape_hs5_sep_n.csv")
w5_tweets <- read.csv(text = w5)

# loading scraped tweets, wave 6 = attack on Berlin, Germany in September (17th)
w6 <- getURL("https://raw.githubusercontent.com/lenniroo/CausalAnalysis_TermPaper/master/Scrape_hs6_sep_n.csv")
w6_tweets <- read.csv(text = w6)

# loading scraped tweets, wave 7 = attack on Parramatta City, Australia in october (2nd)
w7 <- getURL("https://raw.githubusercontent.com/lenniroo/CausalAnalysis_TermPaper/master/Scrape_hs7_oct_n.csv")
w7_tweets <- read.csv(text = w7)

# loading scraped tweets, wave 8 = attack on Paris, France in november (13th)
w8 <- getURL("https://raw.githubusercontent.com/lenniroo/CausalAnalysis_TermPaper/master/Scrape_hs8_nov_n.csv")
w8_tweets <- read.csv(text = w8)

# loading scraped tweets, wave 9 = attack on San Bernardino, USA in december (2nd)
w9 <- getURL("https://raw.githubusercontent.com/lenniroo/CausalAnalysis_TermPaper/master/Scrape_hs9_dec_n.csv")
...
tweets <- w1_tweets %>%
  ...
  bind_rows(w7_tweets) %>%
  bind_rows(w8_tweets) %>%
  bind_rows(w9_tweets) %>% # 126089
  distinct()               # 117836, double tweets: 8253
...
removeURL <- content_transformer(function(x) gsub("(f|ht)tp(s?)://", "", x, perl=T)) #lock at the unicode!! must fix S+
